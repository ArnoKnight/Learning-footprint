...
export default {
  ...
  methods: {
    ...
    select (selection, row) {
      if (this.reimIdLists.indexOf(row.reimId) !== -1) {
        this.reimIdLists.splice(this.reimIdLists.indexOf(row.reimId), 1)
      } else {
        this.reimIdLists.push(row.reimId)
      }
      this.$bus.$emit('getreimIdLists', this.reimIdLists)
    },
    SelectAll (selection) {
      selection.forEach(item => {
        if (this.reimIdLists.indexOf(item.reimId) !== -1) {
          this.reimIdLists.splice(this.reimIdLists.indexOf(item.reimId), 1)
        } else {
          this.reimIdLists.push(item.reimId)
        }
        this.$bus.$emit('getreimIdLists', this.reimIdLists)
      })
