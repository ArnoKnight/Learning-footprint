<template>
  <div class="page-box">
    ...
    >
      ...
      <h4 class="dialog-title">新增家庭收入信息</h4>
      <el-form
        class="info-form"
        ...
        ref="addInfoRuleForm"
      >
        <el-form-item label="收入" label-width="110px" prop="incFee">
          <el-input
            v-model.number="addDialog.infoForm.infoFormDate.incFee"
            size="small"
          ></el-input>
        </el-form-item>
    ...
    >
      <template slot="title">
        <h4 class="dialog-title">家庭收入信息修改</h4>
      </template>
      <el-form
        class="info-forms"
      ...
      >
        <el-form-item label="收入" label-width="110px" prop="incFee">
          <el-input
            v-model.number="editDialog.editFormDate.incFee"
            size="small"
          ></el-input>
        </el-form-item>
        <el-form-item class="textarea-item" label="备注" label-width="110px">
    ...
    >
      <template slot="title">
        <h4 class="dialog-title">家庭收入详情</h4>
      </template>
      <el-form
        class="info-forms"
      ...
      >
        <el-form-item label="收入" label-width="110px" prop="incFee">
          <el-input
            v-model="detailDialog.detailFormDate.incFee"
            disabled
            size="small"
          ></el-input>
        </el-form-item>
...
export default {
      ...
      targetTable: [
        {
          ...
          targetTableWidth: '',
          targetTableProp: 'incFee',
          targetTableLabel: '收入金额'
        }, {
          targetTableWidth: '',
  ...
  methods: {
    ...
    selectBtn (item) {
      ...
      this.$http.post(`${this.$http.defaults.baseURL}/inc/selectIncDetailInfoC?hserCertno=${this.selectForm.hserCertno}&hserName=${this.selectForm.hserName}`
      ).then((res) => {
        console.log(res)
        if (item) {
    ...
    addDialogSubmit (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          incDetailInfoCDTO = this.addDialog.infoForm.infoFormDate
          incDetailInfoCDTO.fmId = this.res.fmId
          if (incDetailInfoCDTO.fmId === undefined) {
            return this.$message({
              type: 'error',
              message: '请先查询您要添加的用户信息'
            })
          }
          this.$http.post(`${this.$http.defaults.baseURL}/inc/addIncDetailInfoC`, incDetailInfoCDTO
          ).then((res) => {
            console.log(res)
            if (res.data.code === '-1') {
              this.$message.error({
    ...
    editDialogSubmit (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.post(`${this.$http.defaults.baseURL}/inc/updateIncDetailInfoC`, incDetailInfoCDTO
          ).then((res) => {
            console.log(res)
            if (res.data.code === -1) {
              this.$message.error({
    ...
    deleteBtn (row) {
      this.$confirm('是否删除该条数据?', '提示', {
        ...
        this.$http.post(`${this.$http.defaults.baseURL}/inc/deleteIncDetailInfoCById`, { incId: row.incId, incFee: row.incFee }
        ).then((res) => {
          console.log(res)
          if (res.data.code === -1) {
            this.$message({
